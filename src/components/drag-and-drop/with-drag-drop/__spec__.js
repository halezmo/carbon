...
describe('WithDragDrop', () => {
  ...
  describe('render', () => {
    ...
    it('clones the children with props passed through', () => {
      ...
      expect(child.props().connectDragSource).toEqual(jasmine.any(Function))
      expect(child.props().connectDragPreview).toEqual(jasmine.any(Function))
    });
  });
  ...
  describe('Drag and drop', () => {
    ...
    beforeEach(() => {
      ...
      beginDragContextSpy = jasmine.createSpy().and.returnValue({ index: 1 });
      beginDragPropSpy = jasmine.createSpy().and.returnValue({ index: 1 });
